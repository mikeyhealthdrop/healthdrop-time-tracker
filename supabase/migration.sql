...
-- =====================================================
-- 6. HELPER FUNCTIONS (in public schema)
-- =====================================================
...
-- USERS: only admins can insert/update/delete
...
-- TIME ENTRIES: employees can update their own, admins can update all
...
CREATE POLICY "Admins can manage time entry edits"
  ...
  USING (
    EXISTS (
      ...
      AND te.org_id = public.get_user_org_id()
    )
    AND public.get_user_role() = 'admin'
  );

-- Allow admins/managers to view edits
...
-- =====================================================
-- 9. FUNCTION: Auto-create user profile on signup
-- =====================================================
...
BEGIN
  ...
  SELECT id INTO healthdrop_org_id FROM public.organizations WHERE slug = 'healthdrop';
  ...
  IF NOT FOUND THEN
    INSERT INTO public.users (auth_id, org_id, email, first_name, last_name, role, employee_type)
    VALUES (
      NEW.id,
      healthdrop_org_id,
      NEW.email,
...
-- =====================================================
-- 10. SEED DATA: Sample jobs for Healthdrop
-- =====================================================
DO $$
DECLARE
  healthdrop_org_id UUID;
